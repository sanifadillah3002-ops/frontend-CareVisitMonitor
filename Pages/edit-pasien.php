...
// Fetch patient data
$patientRes = callAPI('GET', '/patients/' . urlencode($patientId));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = callAPI('PUT', '/patients/' . urlencode($patientId), $payload);
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-section-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        <label for="family_phone" class="form-label">No. HP Keluarga <span style="color:#FF3B30;">*</span></label>
                                        <input type="tel" name="family_phone" id="family_phone" class="form-control"
                                            value="<?= htmlspecialchars($data['family_phone'] ?? '') ?>" required>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    document.getElementById('confirmDelete').addEventListener('click', function() {
        // Send DELETE via AJAX (since HTML forms don't support DELETE)
        const patientId = <?= json_encode($patientId) ?>;
        fetch('../config.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        // Redirect via PHP action — use a hidden form trick
